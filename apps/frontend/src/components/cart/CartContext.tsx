...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        const fetchCart = async () => {
            ...
            setLoading(true);
            try {
                const response = await fetch('https://agrolink-updated-2-4.onrender.com/api/cart', {
                    headers: {
    ...
    setLoading(true);
    try {
        const response = await fetch(`https://agrolink-updated-2-4.onrender.com/api/cart/${productId}`, { 
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`,
            },
